...
final deviceApiProvider = Provider<DeviceApi>(
  (ref) => DeviceApi(ref.read(apiClientProvider)),
);
final deviceRepositoryProvider = Provider<DeviceRepository>(
  (ref) => DeviceRepository(ref.read(deviceApiProvider)),
);
...
class DevicesByRoomNotifier extends FamilyAsyncNotifier<List<dynamic>, int> {
  ...
  Future<void> toggleDeviceStatus(int deviceId, bool currentStatus) async {
    ...
    try {
      await ref
          ...
          .updateDeviceStatus(deviceId, !currentStatus);
    } catch (e) {
      state = previousState;
      rethrow;
    }
  ...
  Future<void> addDevice({required String name, required String type}) async {
    ...
    try {
      await ref
          .read(deviceRepositoryProvider)
          .createDevice(roomId: arg, name: name, type: type, status: true);
      ref.invalidateSelf();
    } catch (e) {
      ref.invalidateSelf();
      rethrow;
...
final devicesByRoomProvider =
    AsyncNotifierProviderFamily<DevicesByRoomNotifier, List<dynamic>, int>(
