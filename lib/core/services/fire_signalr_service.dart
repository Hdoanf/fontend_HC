...
class FireSignalRService {
  ...
  final String _serverUrl = dotenv.get(
    'SIGNALR_URL',
    fallback: "http://13.250.103.252:5020/temperatureHub",
  );
  ...
  Stream<(String, double)> get tempDisplayStream =>
      _tempDisplayController.stream;
  ...
  void init() {
    ...
    _hubConnection!.onreconnecting(
      ({error}) => _connectionStatusController.add("Reconnecting"),
    );
    _hubConnection!.onreconnected(
      ({connectionId}) => _connectionStatusController.add("Connected"),
    );
    _hubConnection!.onclose(
      ({error}) => _connectionStatusController.add("Disconnected"),
    );
    ...
    _hubConnection!.on(
      "FireAlert",
      (args) => _handleIncomingData(args, forceAlert: true),
    );
...
final temperatureStreamProvider = StreamProvider<(String, double)>(
  (ref) => ref.watch(fireSignalRServiceProvider).tempDisplayStream,
);
final fireAlertStreamProvider = StreamProvider<(String, double)>(
  (ref) => ref.watch(fireSignalRServiceProvider).fireEventStream,
);
final connectionStatusProvider = StreamProvider<String>(
  (ref) => ref.watch(fireSignalRServiceProvider).connectionStatusStream,
);
